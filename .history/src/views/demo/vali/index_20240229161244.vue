<template>
  <a-form class="w-5/12" :model="formState" @finish-failed="onFinishFailed" @finish="onFinish">
    <a-row>
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const handleSubmit = () => {
        if (!selectedRadioId.value && !formItems.value.inputValue) {
          alert('不要为空！')
        } else if (!selectedRadioId.value) {
          alert('请选择正确答案！')
        } else if (!formItems.value.inputValue) {
          alert('请填写选项内容!')
        }
