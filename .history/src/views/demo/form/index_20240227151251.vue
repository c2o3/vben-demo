  ...
  <a-form>
    <a-row>
      <a-radio-group v-model:value="selectedRadioId">
        <a-col :span="24" v-for="item in formItems" :key="item.id">
          <a-radio :value="item.id">{{ item.radioLabel }}</a-radio>
          <a-input v-model:value="item.inputValue" style="width: 200px; margin-left: 12px" />
        </a-col>
      </a-radio-group>
    </a-row>
...
<script>
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const handleSubmit = () => {
        console.log(formItems.value, selectedRadioId.value.dep._rawValue)
        return
      }
