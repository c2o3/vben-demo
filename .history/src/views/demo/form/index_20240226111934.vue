        ...
        <a-col :span="2.5">
          <a-form-item>
            <a-radio-group
              ...
              @change="() => handleRadioChange(item.id, item.model)"
            >
              <a-radio :value="true" @click="() => handleRadioChange(item.id, item.model)">{{
                item.radioLabel
              }}</a-radio>
            </a-radio-group>
          </a-form-item>
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const handleRadioChange = async (id, model) => {
        // 重置其他所有单选框的值
        ...
        // 确保DOM更新完成后再获取值
        await nextTick()
        const inputValue = formModel[model]
        console.log(inputValue) // 现在这里应该能获取到最新的值了
      }
